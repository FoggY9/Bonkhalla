...
const run = (client, interaction, options) => {
    ...
    if (targetClan == 'bed')
        var target = bed;
    else if (targetClan == 'per')
        var target = per;
    else if (targetClan == 'khu')
        var target = khu;
    else if (targetClan == 'fiv')
        var target = fiv;
    else if (targetClan == '7t1')
        var target = _7t1;
    else if (targetClan == 'azu')
        var target = azu;
    else if (targetClan == 'str')
        var target = str;
    else {
        return interaction.reply({ content: 'cant find the clan \n Usage: *^claninfo <prefix>* \n first 3 letter of a clan name is its prefix.', ephemeral: false });
    }
    // Get Clan Name
