...
exports.default = (client, oldState, newState) => {
    ...
    if (oldState.channel !== newState.channel && newState.channel === null) {
        if (oldState.member.id == client.user.id)
            setTimeout(() => { joinvc(); }, 2000); //if bot is dc's
        // if has vc record
    ...
    function CreateVc(user, name, userlimit) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield user.guild.channels.create({ name: name, type: discord_js_1.ChannelType.GuildVoice, parent: Category, userLimit: userlimit, rtcRegion: 'singapore' })
                .then((vc) => __awaiter(this, void 0, void 0, function* () {
                client.jointocreatemap.set(`tempvc_${vc.guild.id}_${vc.id}`, vc.id);
                try {
                    yield user.setChannel(vc);
                }
                catch (error) {
                    setTimeout(() => { checkVc(vc); }, 1000 * 2);
                }
            })).catch((err) => console.log(err));
        });
    }
