...
const run = (client, interaction, options) => {
    ...
    if (targetClanPrefix == 'Bedroom Community')
        ...
        target = XUnbeatablesX;
    let accessDn = new discord_js_1.EmbedBuilder().setColor('#FF0000').setDescription(`❌ **|** ${interaction.user} You can't remove roles of members, you dont have permission \nrequired-role: **Clan Leader**`);
    ...
    let done = new discord_js_1.EmbedBuilder().setColor('#0000FF').setDescription(`🔵 **|** **${membr.user.username + '#' + membr.user.discriminator}** has removed **${options._hoistedOptions[0].value}** Clan Role`).setTimestamp()
    ...
    if (!membr.roles.cache.has(target)) {
        ...
        membr.roles.remove(interaction.guild.roles.cache.get(target)).then(interaction.reply({ embeds: [done], ephemeral: false }));
    }
};
exports.run = run;
