...
function execute(client, message, args) {
    ...
    // Created Embed
    let embed = new discord_js_1.EmbedBuilder().setColor('#FFFFFF');
    // Get Targeted Clan
    if (args[0] == 'bed'.toLowerCase())
        var target = bed;
    else if (args[0] == 'per'.toLowerCase())
        var target = per;
    else if (args[0] == 'khu'.toLowerCase())
        var target = khu;
    else if (args[0] == 'fiv'.toLowerCase())
        var target = fiv;
    else if (args[0] == '7t1'.toLowerCase())
        var target = _7t1;
    else if (args[0] == 'azu'.toLowerCase())
        var target = azu;
    else if (args[0] == 'str'.toLowerCase())
        var target = str;
    else {
        return message.channel.send({ content: 'cant find the clan \n Usage: *^claninfo <prefix>* \n first 3 letter of a clan name is its prefix.' });
    }
    // Get Clan Name
    ...
    targetRole.members.forEach((mmbr) => { memberList.push(mmbr.id); });
    // Mark Leaders
    let leaderString = '';
    targetRole.members.forEach((mmbr) => {
        if (mmbr.roles.cache.has('851722247206600704'))
            leaderString = leaderString + ` <@!${mmbr.id}>`;
    });
    // Leaders Field
